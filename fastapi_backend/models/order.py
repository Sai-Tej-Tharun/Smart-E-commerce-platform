...
class Order(Base):
    ...
    # NEW (Refund & Return milestone): when order_status became DELIVERED —
    # set by django_admin the moment an admin marks it delivered (see
    # django_admin/storefront/signals.py), never by FastAPI. Nullable
    # because orders delivered before this column existed won't have it;
    # routes/orders.py's return-window check falls back to updated_at for those.
