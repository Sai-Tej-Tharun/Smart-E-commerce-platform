...
The three mutating endpoints (add/update/remove) are `async def` — not
because they need to be for the DB work, but because they each `await
broadcast_cart_updated(...)` afterward to push a cart_updated WebSocket
event (core/realtime.py). GET /cart stays a plain `def` since it doesn't
push anything.
"""
...
async def add_to_cart(payload: CartItemAdd, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    db.commit()
    cart = _build_cart_response(current_user, db)
    await broadcast_cart_updated(current_user.id, cart)  # so a second open tab updates without polling
    return cart


@router.put("/update", response_model=CartOut)
async def update_cart_item(payload: CartItemUpdate, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    item = (
        db.query(CartItem)
    ...
    db.commit()
    cart = _build_cart_response(current_user, db)
    await broadcast_cart_updated(current_user.id, cart)
    return cart
